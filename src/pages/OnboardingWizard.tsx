...
type Step = "profile" | "goals" | "disclaimer" | "plan";
...
export default function OnboardingWizard({ onDone }: { onDone: () => void }) {
  ...
  const runFeasibility = async () => {
    ...
    try {
      ...
      setCheckError(e?.message || "Errore durante la verifica");
    }
    setChecking(false);
  };
  ...
  const generatePlan = async () => {
    ...
    try {
      ...
      setPlanError(e?.message || "Errore generazione piano");
    }
    setGenerating(false);
  };
  ...
  return (
    <div style={{ maxWidth: "560px", margin: "0 auto", padding: "24px 20px 120px" }}>
      <div style={{ display: "flex", gap: "6px", marginBottom: "20px" }}>
        {(["profile", "goals", "disclaimer", "plan"] as Step[]).map((s, i) => (
          <div key={s} style={{
            flex: 1, height: "4px", borderRadius: "2px",
            background: step === s ? "#E8553A" : ["profile","goals","disclaimer","plan"].indexOf(step) > i ? "#E8553A88" : "#1A1A2E",
          }} />
        ))}
      </div>
      ...
      {step === "profile" && (
        <div style={{ display: "flex", flexDirection: "column", gap: "16px" }}>
          <div>
            <div style={{ fontSize: "11px", fontWeight: 700, letterSpacing: "0.15em", color: "#E8553A", textTransform: "uppercase", fontFamily: "'JetBrains Mono', monospace" }}>Step 1 di 4</div>
            <h2 style={{ fontSize: "26px", fontWeight: 900, margin: "6px 0 4px", letterSpacing: "-0.03em" }}>Chi sei</h2>
            <p style={{ color: "#94A3B8", fontSize: "14px", margin: 0 }}>Mi servono alcuni dati per costruire un piano realistico. Tutto resta sul tuo dispositivo.</p>
          </div>
      ...
      {step === "goals" && (
        <div style={{ display: "flex", flexDirection: "column", gap: "16px" }}>
          <div>
            <div style={{ fontSize: "11px", fontWeight: 700, letterSpacing: "0.15em", color: "#E8553A", textTransform: "uppercase", fontFamily: "'JetBrains Mono', monospace" }}>Step 2 di 4</div>
          ...
          {!hasApiKey() && (
            <div style={{ ...cardStyle, border: "1px solid #F59E0B66", background: "#F59E0B15" }}>
              ...
              <div style={{ fontSize: "13px", color: "#CBD5E1" }}>Per far valutare gli obiettivi al coach devi configurare la chiave API Gemini nelle Impostazioni. Puoi anche saltare e aggiungere obiettivi dopo.</div>
            </div>
          )}
            ...
            <div style={{ display: "flex", flexDirection: "column", gap: "10px" }}>
              {goals.map((g, i) => (
                <div key={g.id} style={cardStyle}>
                  <div style={{ fontSize: "11px", color: "#64748B", marginBottom: "4px" }}>OBIETTIVO {i + 1}</div>
                  ...
                  <button onClick={() => setGoals(gs => gs.filter(x => x.id !== g.id))} style={{ ...ghostBtn, marginTop: "10px", padding: "6px 12px", fontSize: "12px" }}>Rimuovi</button>
                </div>
              ))}
            </div>
          )}
          ...
          {!pendingCheck ? (
            <div style={cardStyle}>
              ...
              {checkError && <div style={{ color: "#EF4444", fontSize: "13px", marginTop: "10px" }}>{checkError}</div>}
            </div>
          ) : (
            <div style={{ ...cardStyle, border: pendingCheck.realistic ? "1px solid #22C55E66" : "1px solid #F59E0B66" }}>
              <div style={{ fontSize: "11px", fontWeight: 700, letterSpacing: "0.1em", color: pendingCheck.realistic ? "#22C55E" : "#F59E0B", textTransform: "uppercase", marginBottom: "8px" }}>
                {pendingCheck.realistic ? "Obiettivo realistico" : "Controproposta del coach"}
              </div>
              ...
              <div style={{ background: "#1A1A2E", padding: "12px 14px", borderRadius: "10px", marginBottom: "12px" }}>
                <div style={{ fontSize: "11px", color: "#64748B", marginBottom: "4px" }}>VERSIONE PROPOSTA</div>
              ...
              <div style={{ display: "flex", gap: "10px" }}>
                <button onClick={acceptProposal} style={{ ...primaryBtn, flex: 1, padding: "12px" }}>Accetta</button>
      ...
      {step === "disclaimer" && (
        <div style={{ display: "flex", flexDirection: "column", gap: "16px" }}>
          <div>
            <div style={{ fontSize: "11px", fontWeight: 700, letterSpacing: "0.15em", color: "#E8553A", textTransform: "uppercase", fontFamily: "'JetBrains Mono', monospace" }}>Step 3 di 4</div>
      ...
      {step === "plan" && (
        <div style={{ display: "flex", flexDirection: "column", gap: "16px" }}>
          <div>
            <div style={{ fontSize: "11px", fontWeight: 700, letterSpacing: "0.15em", color: "#E8553A", textTransform: "uppercase", fontFamily: "'JetBrains Mono', monospace" }}>Step 4 di 4</div>
          ...
          {!plan && !generating && (
            <button onClick={generatePlan} disabled={!hasApiKey()} style={{ ...primaryBtn, opacity: hasApiKey() ? 1 : 0.5 }}>
              {hasApiKey() ? "Genera piano iniziale" : "Configura prima la chiave Gemini in Impostazioni"}
            </button>
          )}
          {generating && (
          ...
          {planError && <div style={{ color: "#EF4444", fontSize: "13px" }}>{planError}</div>}
